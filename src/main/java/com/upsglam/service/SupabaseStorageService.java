...
public class SupabaseStorageService {
	...
	public Mono<String> uploadImage(String userUid, MultipartFile file) {
		...
		return Mono.fromCallable(() -> file.getBytes()).subscribeOn(Schedulers.boundedElastic())
				.flatMap(bytes -> supabaseWebClient.post()
						// Supabase Storage upload endpoint: /storage/v1/object/{bucket}/{path}
						.uri(uriBuilder -> uriBuilder.path("/storage/v1/object/{bucket}/{path}").build(bucketName,
								storagePath))
						.contentType(MediaType.APPLICATION_OCTET_STREAM).bodyValue(bytes).retrieve()
